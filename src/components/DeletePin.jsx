...
const DeletePin = ({ pin, setIsDelete }) => {
  ...
  const deletePin2 = async (pin) => {
    const pinId = pin.idd;
    const res = await PinServices.deletePin(pinId, token);
    if (res.success === true) {
    ...
    <div className="delete-container  ">
      <div className="delete-container-second">
        ...
        <div className="delete-caution">
          <h2 style={{ color: "black", margin: "5px" }}>
            Are you sure do you want to delete pin.
          </h2>
          <button
            style={{ width: "150px" }}
            type="action"
            className="submitButton"
            onClick={() => deletePin2(pin)}
          >
            Delete Pin
          </button>
        </div>
